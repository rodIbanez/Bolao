...
const Leaderboard: React.FC<LeaderboardProps> = ({ lang, matches }) => {
              ...
              <tr key={user.email} className={`group hover:bg-slate-50/50 transition-colors ${index === 0 ? 'bg-yellow-50/30' : ''}`}>
                ...
                <td className="px-2 py-5">
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-xl bg-slate-100 flex items-center justify-center text-slate-400 font-bold overflow-hidden border border-slate-200 shadow-sm">
                      {user.photo ? (
            ...
            {rankings.length === 0 && (
              <tr>
                <td colSpan={3} className="px-6 py-20 text-center text-slate-400 font-medium">
                  Aguardando os primeiros palpites...
                </td>
              </tr>
            )}
          </tbody>
